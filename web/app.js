...
function returnToProse() { const prose = phases[0]; clearPastStepActions(); phaseIndex = 1; validationPassed = false; messages.insertAdjacentHTML('beforeend', `<article class="message assistant-message completion"><div class="avatar">AI</div><div><p>${prose.message}</p><button class="generated-file" type="button" data-file="${prose.file}"><span class="done-icon">✓</span> 已生成 <strong>${prose.label}</strong><span class="open-arrow">打开 ›</span></button><div class="message-actions"><button type="button" data-action="undo">撤回</button><button type="button" data-action="retry">重试</button><button type="button" data-action="next">下一步</button><button type="button" data-action="draft">草稿</button><button type="button" data-action="score">评分</button></div></div></article>`); updatePhaseHeader(); messages.scrollTop = messages.scrollHeight; showToast('已返回正文阶段'); }
function advanceFlow() { if (phaseIndex === 2) { if (!validationPassed) returnToProse(); else { document.querySelector('#chapterPicker').childNodes[0].nodeValue = '第 4 章：未命名 '; document.querySelector('.chat-title h1').textContent = '第 4 章：未命名'; phaseIndex = 0; validationPassed = false; updatePhaseHeader(); showToast('已跳转到第 4 章'); } return; } const phase = phases[phaseIndex]; phaseIndex += 1; const actions = phaseIndex === 2 ? '<button type="button" data-action="undo">撤回</button><button type="button" data-action="retry">重试</button><button type="button" class="confirmed" data-action="pass">通过</button><button type="button" data-action="previous">上一步</button>' : `<button type="button" data-action="undo">撤回</button><button type="button" data-action="retry">重试</button><button type="button" data-action="next">下一步</button>${phase.name === '台词' ? '<button type="button" data-action="draft">草稿</button><button type="button" data-action="score">评分</button>' : ''}`; const messages = document.querySelector('#messages'); clearPastStepActions(); messages.insertAdjacentHTML('beforeend', `<article class="message assistant-message completion"><div class="avatar">AI</div><div><p>${phase.message}</p><button class="generated-file" type="button" data-file="${phase.file}"><span class="done-icon">✓</span> 已生成 <strong>${phase.label}</strong><span class="open-arrow">打开 ›</span></button><div class="message-actions">${actions}</div></div></article>`); updatePhaseHeader(); messages.scrollTop = messages.scrollHeight; showToast(`已进入${phases[phaseIndex].name}阶段`); }
...
(() => {
  ...
  async function scoreProse(project, item) {
    try {
      const response = await fetch('/api/score', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({project:project.diskName, file:`${item.name}.txt`}) });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || '评分失败');
      const number = chapterNumber(item.name), scoreName = `第${number}章评分.md`;
      const report = `# ${scoreName.replace('.md','')}\n\n> 由 Inkflow SLOP 评分脚本生成\n\n\`\`\`text\n${data.report}\n\`\`\``;
  ...
  function card(text, file, group, validation=false, draft=false) { const actions=validation?'<button type="button" data-new-action="undo">撤回</button><button type="button" data-new-action="retry">重试</button><button type="button" class="confirmed" data-new-action="pass">通过</button><button type="button" data-new-action="previous">上一步</button>':`<button type="button" data-new-action="undo">撤回</button><button type="button" data-new-action="retry">重试</button><button type="button" data-new-action="next">下一步</button>${draft?'<button type="button" data-new-action="draft">草稿</button><button type="button" data-new-action="score">评分</button>':''}`; clearPastStepActions(); messages.insertAdjacentHTML('beforeend',`<article class="message assistant-message completion"><div class="avatar">AI</div><div><p>${text}</p>${file?`<button class="generated-file" type="button" data-new-file="${file}" data-new-group="${group}"><span class="done-icon">✓</span> 已生成 <strong>${file}</strong><span class="open-arrow">打开 ›</span></button>`:''}<div class="message-actions">${actions}</div></div></article>`); messages.scrollTop=messages.scrollHeight; }
  ...
  function runChapter(project) { const item=chapter(project), stage=chapterFlow[item.index], file=stage.file||`${item.name}.txt`; add(project,stage.group,file);project.content[file]=`# ${file.replace(/\.(md|txt)$/,'')}\n\n此产物由“${stage.name}”步骤生成。`;persist(project,file,project.content[file]);card(`${stage.name}已生成。`,file,stage.group,item.index===chapterFlow.length-1,stage.name==='正文');if(item.index<chapterFlow.length-1)item.index+=1;else item.checked=true;sync(project,stage.group);header(project);menu(project); }
