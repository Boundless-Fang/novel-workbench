...
/* 本章配置的受控选项：[中文分组, 选项, 单/多选, 后端字段名]，与 工作流脚本/步骤定义.py 的 CONFIG_OPTIONS 对应。 */
const configGroups = [
  ['叙事视角',['第一人称','第三人称'],'single','person'],
  ['叙事结构',['顺叙','倒叙','插叙'],'multi','narrative'],
  ['结构模板',['铺垫蓄势','冲突递进','悬念收束'],'single','structure'],
  ['事件评级',['主线','支线','闲笔'],'single','event_level'],
  ['场景组织',['单场景','多场景'],'single','scene'],
  ['信息安排',['背景设定','场景氛围','前置剧情','人物出场','信息揭示','身份揭示','回忆/前情','冲突','伏笔','悬念','回应/揭示','对话引入','场景切换'],'multi','information'],
  ['人物变化',['性格对比','心理状态','关系变化','结尾状态'],'multi','changes'],
  ['事件要素',['误会','危机','反转','和解'],'multi','events'],
  ['表达方式',['叙述','描写','抒情','议论','说明'],'multi','expression'],
  ['行为触发',['对话/声响触发','情绪触发','对比','延宕','因果链'],'multi','triggers']
];
const fileState = {};
...
function renderFiles(group) { fileList.innerHTML = files[group].map(key => { const label = group==='正文' ? (window.novelLocal?.chapterDisplay?.(baseName(key)) || baseName(key)) : baseName(key); return `<button class="file-row ${key === currentFile ? 'active' : ''}" data-file="${key}"><span class="file-glyph"></span>${label}</button>`; }).join(''); }
function renderTabs() { documentTabs.innerHTML = openFiles.map(key => { const label=baseName(key); return `<button class="document-tab ${key === currentFile ? 'active' : ''}" data-tab="${key}"><span>${label}</span><span class="tab-close" data-close="${key}" aria-label="关闭 ${label}">×</span></button>`; }).join(''); }
...
/* 打开的文件以“唯一键”标识：默认是文件名；与已有文件重名时（如各章节的 台词.md）改用完整路径，
   避免同名文件在 project.paths / project.content 中互相覆盖。 */
function openFile(rawName) {
  const raw=String(rawName||''), name=baseName(raw);
  window.novelSettings?.openWorkspaceForFile?.();
  if (!openFiles.includes(raw)) openFiles.push(raw);
  currentFile = raw;
  const project=window.novelLocal?.active?.();
  let filePath=project?.paths?.[raw] || '';
  if(!filePath && project?.paths){
    const hit=Object.entries(project.paths).find(([key, path])=>key===name || key===`${name}.md` || path===raw || path.endsWith(`/${name}`) || path.endsWith(`/${name}.md`));
    if(hit) filePath=hit[1];
  }
  if(!filePath && (raw.includes('/') || raw.includes('\\'))) filePath=raw.replace(/\\/g,'/');
  path.textContent = `当前文件 / ${name}`; renderTabs(); document.querySelectorAll('.file-row').forEach(row => row.classList.toggle('active', row.dataset.file === raw));
  if(filePath){
    content.value = fileState[raw] ?? '';
    setEditorMode(false);
    fetch(`/api/file?project=${encodeURIComponent(project.diskName)}&path=${encodeURIComponent(filePath)}`).then(r=>r.ok?r.json():null).then(data=>{
      if(data && typeof data.content === 'string'){
        fileState[raw]=data.content;
        project.content[raw]=data.content;
        if(currentFile===raw){ content.value=data.content; setEditorMode(false); }
      }
    }).catch(()=>{ if(fileState[raw]===undefined){ content.value=`# ${name}\n\n文件不存在或读取失败。`; setEditorMode(false); } });
  } else { content.value = fileState[raw] ?? `# ${name}\n\n文件不存在。`; setEditorMode(false); }
}
...
function recordWorkflowFailure(task, label, error, retryable=true) {
  ...
  /* 重试走 data-new-action="next"：与「下一步」同路，重新执行当前阶段；阶段未推进，重跑即重试。 */
  const retryHtml=retryable ? `<div class="message-actions"><button type="button" data-new-action="next">重试当前阶段</button></div>` : '';
  article.className='message assistant-message workflow-failure';
  article.innerHTML=`<div class="avatar">${isValidation ? '✗' : '!'}</div><div><p><strong>${title}</strong></p><p class="form-note"></p>${usageHtml}<details open><summary>错误详情（已保留）</summary><pre></pre></details>${retryHtml}</div>`;
...
document.querySelector('#searchForm').addEventListener('submit', event => { event.preventDefault(); const q = document.querySelector('#searchInput').value.trim().toLowerCase(); const results = Object.entries(files).flatMap(([group, keys]) => keys.filter(key => key.toLowerCase().includes(q)).map(key => ({ group, key }))); document.querySelector('#searchResults').innerHTML = q ? (results.length ? results.map(item => `<button class="search-result" type="button" data-result="${item.key}" data-result-group="${item.group}">${item.group} / ${baseName(item.key)}</button>`).join('') : '<span>没有匹配的文件</span>') : '<span>请输入关键词后点击“查找”</span>'; });
...
document.querySelector('.project-name-label').insertAdjacentHTML('afterend', '<label class="fan-source hidden" id="fanSource">上传原著文件（.txt、.md）<input type="file" id="sourceFile" accept=".txt,.md,text/plain,text/markdown" /></label>');
...
(() => {
  ...
  const add = (project, group, path) => { const name=baseName(path); let key=name; if (project.paths[name] && project.paths[name] !== path) key=String(path); if (!project.files[group].includes(key)) project.files[group].push(key); if (!project.paths[key]) project.paths[key] = String(path); return key; };
  ...
  async function viewExistingFile() { const pending=pendingExistingFile; if (!pending) return; try { const response=await fetch(`/api/file?project=${encodeURIComponent(pending.project.diskName)}&path=${encodeURIComponent(pending.path)}`), data=await response.json(); if (!response.ok) throw new Error(data.error || '无法读取文件'); const group=groupForPath(pending.path); const key=add(pending.project,group,pending.path); pending.project.content[key]=data.content; closeExistingFileModal(); sync(pending.project,group); openFile(key); } catch (error) { showToast(`无法打开本地文件：${error.message}`); } }
  async function useExistingInitializationFile(project, expected, stage) {
    try {
      ...
      const key=add(project,group,expected); project.content[key]=data.content;
  ...
  async function persist(project, name, value, customPath='') {
    ...
    // name 可能是“唯一键”（重名文件的完整路径）：带 / 时直接当作保存路径。
    const path = customPath || (name.includes('/') ? name : localPath(project, name));
    project.paths ||= {};
  ...
  function saveConversation(project) { const snapshot=messages.cloneNode(true); snapshot.querySelectorAll('.generation-pending').forEach(node=>node.remove()); const nodes=[...snapshot.children]; while(nodes.length>1 && new Blob([snapshot.innerHTML]).size>1_800_000) nodes.shift().remove(); project.conversation=snapshot.innerHTML; return persist(project,'对话记录.md',project.conversation,'运行记录/对话记录.md'); }
  ...
  async function saveWorkflowState(project) {
    ...
    const state = {
      ...
      chapterId: chapter(project)?.name || null,
  ...
  function prosePathOf(project, chapterName) {
    const name = `${chapterName}.txt`;
    if (project.paths?.[name]) return project.paths[name];
    const hit = Object.entries(project.paths || {}).find(([, p]) => p.endsWith(`/${name}`));
    return hit?.[1] || `正文/${name}`;
  }
  async function saveDraft(project, item) {
    const prosePath = prosePathOf(project, item.name), source = project.content[`${item.name}.txt`] !== undefined ? `${item.name}.txt` : Object.keys(project.content).find(key => project.paths?.[key] === prosePath) || `${item.name}.txt`;
    const number = chapterNumber(item.name);
  ...
  async function scoreProse(project, item) {
    try {
      const prosePath = prosePathOf(project, item.name);
  ...
  /* 对账式同步：只刷新文件清单与内容映射，不重置右栏编辑器状态。
     仅当当前打开的文件已被删除（撤回/重置）时才关闭对应标签页；
     正在编辑时绝不覆盖输入区内容。 */
  function sync(project, group = project.group) {
    Object.entries(files).forEach(([key, value]) => value.splice(0, value.length, ...project.files[key]));
    Object.keys(fileState).forEach(key => { if (!Object.hasOwn(project.content, key)) delete fileState[key]; });
    Object.assign(fileState, project.content);
    openFiles = openFiles.filter(name => Object.hasOwn(fileState, name));
    if (currentFile && !Object.hasOwn(fileState, currentFile)) currentFile = openFiles[openFiles.length - 1] || '';
    if (currentFile) {
      if (!editing) { content.value = fileState[currentFile]; setEditorMode(false); }
      path.textContent = `当前文件 / ${baseName(currentFile)}`;
    } else { content.value=''; preview.innerHTML=''; path.textContent='当前文件 / 未选择'; }
    renderTabs(); renderFiles(group);
  }
  const extractionIds = new Set(['text_stats','word_frequency','style','positive_vocabulary','exclusive_vocabulary']);
  function updateStepProgress(project, item) {
    const target=document.querySelector('#stepProgress'), chips=document.querySelector('#fanExtractChips');
    if(!project.initialized){
      const flow=stages(project), completed=new Set(flow.slice(0,project.initIndex || 0).map(stage=>stage.id));
      // 同人项目：五个原著提取步骤单独显示在标题行 ••• 右侧，第二行进度条与原创保持一致。
      if (chips) {
        const showChips = project.type === '同人' && flow.some(stage => stage.id && extractionIds.has(stage.id));
        chips.classList.toggle('hidden', !showChips);
        if (showChips) chips.innerHTML = flow.filter(stage => stage.id && extractionIds.has(stage.id)).map(stage => `<span class="${completed.has(stage.id) ? 'done' : ''}">${stage.name}</span>`).join('');
      }
      target.innerHTML=flow.filter(stage=>stage.id && !extractionIds.has(stage.id)).map(stage=>`<span class="${completed.has(stage.id) ? 'done' : ''}">${stage.name}</span>`).join('');
      return;
    }
    if (chips) { chips.classList.add('hidden'); chips.innerHTML=''; }
    const enabled=chapterStages(), completed=new Set(item?.approved ? enabled.map(stage=>stage.id) : enabled.slice(0,item?.index || 0).map(stage=>stage.id)); const labels=[['compile_anchor','锚点'],['compile_config','配置'],['compile_dialogue','台词'],['generate_prose','正文'],['validate','校验报告']].filter(([id])=>enabled.some(stage=>stage.id===id)); target.innerHTML=labels.map(([id,label])=>`<span class="${completed.has(id) ? 'done' : ''}">${label}</span>`).join('');
  }
  const chapterLabel = item => item?.displayName || item?.name || '未命名章节';
  function header(project) { let title='初始化', stage, target, next, item; if (!project.initialized) { const flow=stages(project), index=project.initIndex || 0; if(index===0 && project.type!=='同人'){stage={id:'compile_intro',name:'小说资料'};target=project.introResult ? flow[0] : stage;next=project.introResult?'下一步：生成简介':'下一步：判别资料';}else if(index===0){stage=flow[0] || {name:'初始化已完成'};target=stage;next=stage.id?`开始${stage.name}`:'完成初始化';}else{stage=flow[index-1] || {name:'初始化已完成'};target=flow[index] || null;next=target?`下一步：生成${target.name}`:'完成初始化';} } else if (!project.chapterId) { if (project.chapters?.length) project.chapterId = project.chapters[0].id; if (!project.chapterId) { title='准备新章节'; stage={name:'初始化已完成'}; target=stage; next='新建第1章'; } else { item=chapter(project); title=chapterLabel(item); stage=item.approved ? {name:'已通过'} : (chapterStages()[item.index] || {name:'本章已完成'}); target=stage; next=item.approved ? `新建第${project.chapters.length + 1}章` : item.checked ? '上一步' : `下一步：生成${stage.name}`; } } else { item=chapter(project); title=chapterLabel(item); stage=item.approved ? {name:'已通过'} : (chapterStages()[item.index] || {name:'本章已完成'}); target=stage; next=item.approved ? `新建第${project.chapters.length + 1}章` : item.checked ? '上一步' : `下一步：生成${stage.name}`; } document.querySelector('.eyebrow').textContent=`${project.name} /`; document.querySelector('.chat-title h1').textContent=title; const completedInit=!project.initialized && (project.initIndex || 0)>0; const completedChapter=project.initialized && item && item.index>0 && !item.approved ? (chapterStages()[item.index-1] || {name:'已生成'}).name : null; const initDone=project.initialized && !project.chapterId; const statusText=initDone ? '已生成' : completedInit ? `${stage.name} 已生成` : completedChapter ? `${completedChapter} 已生成` : `当前阶段：${stage.name}`; document.querySelector('.workflow').innerHTML=`<span class="status-dot"></span>${statusText}`; document.querySelector('#nextStep').innerHTML=`${next} <span>›</span>`; const prompt=document.querySelector('#prompt'); if(prompt)prompt.placeholder=stage?.id==='compile_intro'&&!project.introResult?'请输入所有与小说相关的信息：角色、背景、设定、剧情等…':stage?.id==='compile_anchor'?'请输入本章信息：已知出场角色 + 本章剧情梗概…':extractionSteps.has(stage?.id)?'原著已上传；点击“开始”即可执行本地提取。':'可补充本步骤的创作信息；发送即生成当前阶段…'; setGenerationTarget(target?.name || '未选择'); updateStepProgress(project,item); document.querySelector('#stageSkip')?.remove(); if (target?.optional) { const skip=document.createElement('button'); skip.type='button'; skip.id='stageSkip'; skip.className='next-step'; skip.textContent=`跳过${target.name}`; skip.addEventListener('click',()=>!project.initialized ? runInitialization(project,true) : skipChapterStage(project)); document.querySelector('.composer-workflow').prepend(skip); } }
  ...
  function card(text, file, group, validation=false, draft=false, task='', retryPayload=null, retryComplete=false, files=[], kind='', usage=null) { const actions=validation?'<button type="button" data-new-action="undo">撤回</button><button type="button" data-new-action="retry">重试</button><button type="button" class="confirmed" data-new-action="pass">通过</button><button type="button" data-new-action="previous">上一步</button>':`<button type="button" data-new-action="undo">撤回</button><button type="button" data-new-action="retry">重试</button><button type="button" data-new-action="next">下一步</button>${draft?'<button type="button" data-new-action="draft">草稿</button><button type="button" data-new-action="score">评分</button>':''}`; const fileList=Array.isArray(file)?file:(file?[file]:[]); const fileButtons=fileList.map(f=>{const label=baseName(f); return `<button class="generated-file" type="button" data-new-file="${f}" data-new-group="${group}"><span class="done-icon">✓</span> 已生成 <strong>${label}</strong><span class="open-arrow">打开 ›</span></button>`;}).join(''); const retryAttrs=task?` data-task="${task}" data-retry-input="${encodeURIComponent(JSON.stringify(retryPayload||{}))}" data-retry-complete="${retryComplete?'1':'0'}"`:''; const fileData=files.length?` data-files="${encodeURIComponent(JSON.stringify(files))}" data-kind="${kind}"`:''; const usageHtml=usage?`<div class="usage-line">模型：${usage.last?.model||''} · 输入 ${usage.prompt_tokens||0} · 输出 ${usage.completion_tokens||0} · 耗时 ${usage.response_time||0}s</div>`:''; clearPastStepActions(); messages.insertAdjacentHTML('beforeend',`<article class="message assistant-message completion"${retryAttrs}${fileData}><div class="avatar">AI</div><div><p>${text}</p>${fileButtons}<div class="message-actions">${actions}</div>${usageHtml}</div></article>`); messages.scrollTop=messages.scrollHeight; return messages.lastElementChild; }
  async function applyGeneratedOutputs(project, task, input, complete, result) {
    ...
    for (const output of outputs) {
      ...
      if (/\.(?:json|log\.md)$/i.test(currentName)) continue;
      ...
      const key = add(project, group, output);
      project.content[key] = data.content;
      generatedFiles.push(key);
    }
    const displayFiles = generatedFiles.length ? generatedFiles : [outputs[0].split('/').pop()];
    const group = displayFiles[0] ? ((project.paths[displayFiles[0]] || displayFiles[0]).split('/')[0] || '知识库') : '知识库';
  ...
  async function undoCompletion(completion) {
    ...
    for (const path of paths) {
      ...
      // 同名文件可能以完整路径为键注册：按路径值清除所有匹配的键。
      for (const [key, value] of Object.entries(project.paths)) {
        if (value !== path) continue;
        delete project.paths[key];
        delete project.content[key];
        const group = path.split('/')[0] || '知识库';
        const idx = (project.files[group] || []).indexOf(key);
        if (idx >= 0) project.files[group].splice(idx, 1);
      }
    }
    const kind = completion?.dataset.kind;
    const isLatestCard = Boolean(kind) && [...document.querySelectorAll(`#messages .completion[data-kind="${kind}"]`)].at(-1) === completion;
    if (kind === 'init' && isLatestCard && project.initIndex > 0) {
      project.initIndex -= 1;
      if (project.initIndex < (stages(project) || []).length) project.initialized = false;
    } else if (kind === 'chapter' && isLatestCard) {
  ...
  async function assessProjectBrief(project) { const input=document.querySelector('#prompt'), brief=input.value.trim(); if(!brief){showToast('请输入小说相关信息');input.focus();return false;} try { showToast('正在判别小说资料…'); const response=await fetch('/api/project-brief/assess',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project:project.diskName,content:brief})}), result=await response.json(); if(!response.ok)throw new Error(result.error||'判别失败'); if(result.missing?.length){const labels={character:'补充主角信息',synopsis:'补充剧情梗概'};showToast(result.missing.map(key=>labels[key]).filter(Boolean).join(' / '),6000);input.focus();return false;} addUsage(project,result.usage); project.introInput=brief; project.introResult=result; project.content['初始化资料.md']='# 初始化资料\n\n## 初始用户信息\n'+brief; await persist(project,'初始化资料.md',project.content['初始化资料.md'],'运行记录/初始化资料.md'); await saveWorkflowState(project); input.value='';autoGrowComposer(); card('小说资料已确认，可生成正式简介与标签。'); header(project); return true; } catch(error) { recordWorkflowFailure('compile_intro','小说资料判别',error);showToast(`小说资料判别失败：${error.message}`,6000);return false; } }
  async function runInitialization(project, skipped=false, structuredInput=null, supplement='', force=false) { const flow=stages(project), stage=flow[project.initIndex]; const stageIndex=project.initIndex; if (!stage) { project.initialized=true; await saveWorkflowState(project); card('初始化已完成。现在可以新建章节。'); header(project); menu(project); return; } if (skipped) { project.initIndex+=1; await saveWorkflowState(project); card(`已跳过${stage.name}。`); sync(project); header(project); menu(project); return; } if (!stage.id) { project.initIndex+=1; await saveWorkflowState(project); return runInitialization(project); } if(stage.id==='compile_intro'&&!structuredInput){if(!project.introResult)return assessProjectBrief(project);structuredInput={summary:project.introResult.summary,tags:project.introResult.tags};} if(extractionSteps.has(stage.id)&&!structuredInput){if(!project.sourceName){showToast('同人项目缺少已上传的原著文件');return;}structuredInput={source:project.sourceName};} if(supplement)await appendInitializationMaterial(project,supplement); if (initInputSpec[stage.id] && !structuredInput) return openInitInput(project,stage); if (stage.id === 'generate_characters_batch') { try { const rosterResponse=await fetch(`/api/file?project=${encodeURIComponent(project.diskName)}&path=${encodeURIComponent('知识库/角色名单.json')}`), rosterData=await rosterResponse.json(); if(!rosterResponse.ok || !rosterData.content) throw new Error('角色名单不存在'); const roster=JSON.parse(rosterData.content), characters=roster.characters||[]; if(!characters.length) throw new Error('角色名单为空'); const skipped=[]; for(const ch of characters){ const chName=String(ch?.name||'').trim(); if(!chName) continue; /* 断点续传：磁盘上已有的角色卡直接注册并跳过，整批重试不再重做。 */ const expectedCard=`知识库/角色卡/角色卡-${chName}.md`; if(await localFileExists(project, expectedCard)){ const existing=await fetch(`/api/file?project=${encodeURIComponent(project.diskName)}&path=${encodeURIComponent(expectedCard)}`).then(r=>r.ok?r.json():null); if(existing&&typeof existing.content==='string'){const existingKey=add(project,'知识库',expectedCard);project.content[existingKey]=existing.content;skipped.push(chName);card(`角色卡 ${chName} 已存在，跳过生成。`,existingKey,'知识库');continue;}} const payload={name:chName}; if(ch?.brief) payload.brief=ch.brief; const result=await window.novelWorkflow.run('generate_character', payload, true); addUsage(project, result.usage); const output=result.outputs?.[0]; if(!output) throw new Error(`角色卡 ${chName} 生成失败`); const data=await fetch(`/api/file?project=${encodeURIComponent(project.diskName)}&path=${encodeURIComponent(output)}`).then(r=>r.ok?r.json():Promise.reject(new Error('无法读取角色卡'))); const key=add(project,'知识库',output); project.content[key]=data.content; card(`角色卡 ${chName} 已生成。`, key, '知识库', false, false, 'generate_character', payload, true, result.outputs, 'init', result.usage); } if(skipped.length)card(`已跳过 ${skipped.length} 张已存在的角色卡：${skipped.join('、')}`); const retryIndex=stageIndex; lastRetry = () => { project.initIndex = retryIndex; return runInitialization(project, false, structuredInput, '', true); }; project.initIndex=stageIndex+1; if(project.initIndex>=flow.length)project.initialized=true; await saveWorkflowState(project); if(project.initialized)card('初始化已完成。现在可以新建章节。'); sync(project); header(project); menu(project); maybeAutoContinue(project); return; } catch(error) { recordWorkflowFailure(stage.id,stage.name,error); showToast(`${workflowFailureTitle(error)}：详情已保留在对话中`,5000); return; } }  const input=structuredInput || {user_supplement:project.content['初始化资料.md'] || ''}, expected=initOutputPath(stage,input); if(!force && expected && await localFileExists(project,expected)) return promptExistingFile(project,expected,()=>runInitialization(project,false,structuredInput,'',true),()=>useExistingInitializationFile(project, expected, stage)); try { const result=await window.novelWorkflow.run(stage.id,input,Boolean(structuredInput)), outputs=result.outputs||[]; addUsage(project, result.usage); if(!outputs.length)throw new Error('流程脚本没有返回正式产物'); const group=stage.output[0]?.[0]||'知识库'; const isHiddenJson=name=>name.toLowerCase().endsWith('.json'); const generatedFiles=[]; let name='';
for(const output of outputs){ const currentName=output.split('/').pop(), data=await fetch(`/api/file?project=${encodeURIComponent(project.diskName)}&path=${encodeURIComponent(output)}`).then(response=>response.ok?response.json():Promise.reject(new Error('无法读取脚本产物'))); if(isHiddenJson(currentName)){ project.paths[currentName]=output; project.content[currentName]=data.content; continue; } const key=add(project,group,output); project.content[key]=data.content; generatedFiles.push(key); if(!name)name=key; }
if(!name)name=outputs[0].split('/').pop(); const displayFiles=generatedFiles.length?generatedFiles:[name]; const retryIndex=stageIndex; lastRetry = () => { project.initIndex = retryIndex; return runInitialization(project, false, structuredInput, '', true); }; project.initIndex=stageIndex+1; if(project.initIndex>=flow.length)project.initialized=true; await saveWorkflowState(project); card(`${stage.name}已由流程脚本生成。${generatedFiles.length>1?`（${generatedFiles.length} 个文件）`:''}`,displayFiles,group,false,false,stage.id,input,Boolean(structuredInput),outputs,'init',result.usage); if(project.initialized)card('初始化已完成。现在可以新建章节。'); sync(project);header(project);menu(project);maybeAutoContinue(project);document.querySelector('#prompt').value='';autoGrowComposer(); } catch(error) { if (error?.usage) { addUsage(project, error.usage); await saveWorkflowState(project); } recordWorkflowFailure(stage.id,stage.name,error); showToast(`${workflowFailureTitle(error)}：详情已保留在对话中`,5000); } }
  ...
  async function runChapter(project, input={}, force=false, inputComplete) { const item=chapter(project), flow=chapterStages(), stage=flow[item.index]; const stageIndex=item.index; if (!stage) { item.checked=true; item.approved=true; await saveWorkflowState(project); return header(project); } const complete = inputComplete === undefined ? stage.id==='compile_anchor' : inputComplete; const expected=chapterOutputPath(item,stage); if(!force && expected && await localFileExists(project,expected)) return promptExistingFile(project,expected,()=>runChapter(project,input,true,complete),()=>skipExistingChapterFile(project)); try { const payload={chapter:item.name,user_supplement:item.chapterUserInfo || item.chapterBrief || '',...input}; const result=await window.novelWorkflow.run(stage.id,payload,complete); addUsage(project, result.usage); const output=result.outputs?.[0]; if(!output)throw new Error('流程脚本没有返回正式产物'); const data=await fetch(`/api/file?project=${encodeURIComponent(project.diskName)}&path=${encodeURIComponent(output)}`).then(response=>response.ok?response.json():Promise.reject(new Error('无法读取脚本产物'))); const file=add(project,stage.group,output); project.content[file]=data.content; if(stage.id==='compile_anchor')item.anchorInput=payload; const retryIndex=stageIndex; lastRetry = () => { item.index = retryIndex; return runChapter(project, input, true, complete); }; if(stageIndex<flow.length-1)item.index=stageIndex+1;else item.checked=true; await saveWorkflowState(project); card(`${stage.name}已由流程脚本生成。`,file,stage.group,stage.id==='validate',stage.name==='正文',stage.id,payload,complete,result.outputs,'chapter',result.usage); sync(project,stage.group);header(project);menu(project);maybeAutoContinue(project); } catch(error) { if (error?.usage) { addUsage(project, error.usage); await saveWorkflowState(project); } recordWorkflowFailure(stage.id,stage.name,error); showToast(`${workflowFailureTitle(error)}：详情已保留在对话中`,5000); } }
  async function assessChapterBrief(project) { const item=chapter(project), input=document.querySelector('#prompt'), brief=input.value.trim(); if(!brief){showToast('请输入本章信息');input.focus();return false;} try { showToast('正在判别本章信息…'); const response=await fetch('/api/chapter-brief/assess',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project:project.diskName,chapter:item.name,content:brief})}), result=await response.json(); if(!response.ok)throw new Error(result.error||'判别失败'); if(result.missing?.length){const labels={known_character:'补充信息卡',synopsis:'补充剧情梗概'}; showToast(result.missing.map(key=>labels[key]).filter(Boolean).join(' / '),6000);input.focus();return false;} addUsage(project,result.usage); await appendChapterUserInfo(project,item,brief,true); item.chapterBriefResult=result; await saveWorkflowState(project); input.value='';autoGrowComposer(); card('本章信息已确认，可进入锚点生成。'); header(project); return true; } catch(error) { recordWorkflowFailure('compile_anchor','章节信息判别',error); showToast(`章节信息判别失败：${error.message}`,6000); return false; } }
  ...
  /* 本章配置：默认用选择式面板确定受控选项，不再依赖模型从自由文本里猜。 */
  function renderConfigPicker() { document.querySelector('#configFields').innerHTML = configGroups.map(([label, options, mode]) => `<div class="choice-group"><span>${label}${mode === 'multi' ? '（可多选）' : '（单选）'}</span><div class="choice-list">${options.map(option => `<button type="button" class="choice" data-cfg-group="${label}" data-cfg-option="${option}" data-cfg-mode="${mode}">${option}</button>`).join('')}</div></div>`).join(''); }
  function openConfigPicker() { renderConfigPicker(); document.querySelector('#configModal').classList.remove('hidden'); document.querySelector('#configFields .choice')?.focus(); }
  document.querySelector('#configFields').addEventListener('click', event => { const choice=event.target.closest('.choice'); if(choice) choice.classList.toggle('selected'); });
  document.querySelector('#configForm').addEventListener('submit', event => {
    event.preventDefault();
    const project=active(), item=project&&chapter(project);
    if(!project||!item)return;
    const values={};
    for(const choice of document.querySelectorAll('#configFields .choice.selected')) {
      const key=configGroups.find(group => group[0] === choice.dataset.cfgGroup)?.[3];
      if(!key)continue;
      if(choice.dataset.cfgMode === 'multi')(values[key] ||= []).push(choice.dataset.cfgOption);
      else values[key]=choice.dataset.cfgOption;
    }
    document.querySelector('#configModal').classList.add('hidden');
    if(!Object.keys(values).length)return showToast('请至少选择一项，或点击“让 AI 依据本章信息推断”');
    runChapter(project, values, false, true);
  });
  document.querySelector('#configAutoInfer').addEventListener('click', () => { document.querySelector('#configModal').classList.add('hidden'); const project=active(); if(project&&chapter(project)) runChapter(project); });
  async function executeCurrentChapter(project, supplement='') { const item=chapter(project),stage=item&&chapterStages()[item.index]; if(!item||!stage)return; if(stage.id==='compile_config'&&!supplement&&!isAutoMode()){if(!document.querySelector('#configModal').classList.contains('hidden'))return;return openConfigPicker();} if(stage.id==='compile_anchor'){const wasAssessed=Boolean(item.chapterBriefResult);if(!wasAssessed && !(await assessChapterBrief(project)))return;if(wasAssessed&&supplement)await appendChapterUserInfo(project,item,supplement);const result=item.chapterBriefResult;return runChapter(project,{characters:result.characters,core_event:result.core_event,information_boundary:'',foreshadowing:'',hook:''});} if(supplement)await appendChapterUserInfo(project,item,supplement); return runChapter(project); }
  ...
  async function runGlobalAsset(project, task, naturalInput) {
    try {
      ...
      const displayFiles=outputs.filter(output=>!/\.json$/i.test(output)).map(output=>{
        const name=output.split('/').pop();
        return project.paths[name] === output ? name : output;
      });
      ...
      if(error?.usage) { addUsage(project, error.usage); await saveWorkflowState(project); }
      recordWorkflowFailure(task, task, error, false);
  ...
  async function performRestoreLocalProjects() {
    try {
      // 等工作台设置就绪，避免“禁用步骤”配置在恢复时未生效导致流程进度错位。
      await window.novelSettings?.ready;
      ...
      for (const entry of entries) {
        ...
        if (/\.(?:json|log\.md)$/i.test(entry.name)) continue;
        const key = add(project, entry.group, entry.path);
        const fileResponse = await fetch(`/api/file?project=${encodeURIComponent(projectName)}&path=${encodeURIComponent(entry.path)}`);
        const fileData = await fileResponse.json();
        if (fileResponse.ok) project.content[key] = fileData.content;
      }
      ...
      // 兼容旧会话：历史版本曾把内部执行日志作为“已生成文件”写入对话。
      messages.querySelectorAll('[data-new-file$=".log.md"]').forEach(button=>button.remove());
  ...
  async function deleteCurrentChapter() {
    ...
    for (const [name,path] of Object.entries(project.paths || {})) if (path===prosePath || path.includes(`/${item.name}/`)) doomed.add(name);
    project.files['正文']=project.files['正文'].filter(key=>!doomed.has(key) && `${key}` !== `${item.name}.txt`);
    for (const name of doomed) { delete project.paths[name]; delete project.content[name]; }
  ...
  window.novelLocal = { active, localPath, prosePathOf, add, saveWorkflowState, restoreLocalProjects, deleteCurrentChapter, selectChapterByFile, chapterDisplay };
...
- 对话：中文引号 “”
  ...
  const settingsReady = read().then(apply);
  window.novelSettings = {
    openWorkspaceForFile: () => { if (settings.openFilesInSidebar) setWorkspaceOpen(true); },
    setWorkspaceOpen,
    // 只过滤设置界面里可控的步骤；不在流程选项清单里的步骤（如同人提取）始终启用，
    // 否则它们永远不在 enabledSteps 白名单中，会被错误地全部过滤掉。
    isStepEnabled: step => !allSteps.includes(step) || settings.enabledSteps.includes(step),
...
(() => {
  ...
  async function run(task, input, inputComplete=false) {
    if (activeRun) throw new Error('已有任务正在运行，请等待完成或点击“终止”');
    const project = window.novelLocal?.active?.();
  ...
  window.novelWorkflow = { run, isRunning: () => Boolean(activeRun) };
  document.addEventListener('novel:run-workflow', async event => {
    ...
    catch (error) { recordWorkflowFailure(event.detail.task,event.detail.task,error,false); showToast(`${workflowFailureTitle(error)}：详情已保留在对话中`,5000); }
  });
})();
...
(() => {
  ...
  async function refreshVolumes(){const item=project();if(!item)return;/* 已有路径优先：只读扫描，不迁移、不移动任何文件。 */const tree=await request(`/api/projects/${encodeURIComponent(item.diskName)}/tree`);const proseRoot=tree.tree.find(node=>node.name==='正文');const volumes=prose(proseRoot).filter(node=>node.type==='directory');if(!activeVolume||!volumes.some(node=>node.name===activeVolume))activeVolume=volumes[0]?.name||'';chapterBox.remove();document.querySelector('#volumeList').innerHTML=volumes.map(node=>`<button type="button" class="volume-row ${node.name===activeVolume?'active':''}" data-volume="${node.name}" style="padding:5px 9px;font-size:12px;line-height:16px"><span class="project-dot"></span><span>${node.name}</span></button>`).join('')||'<span class="form-note">暂无卷</span>';const activeButton=document.querySelector(`#volumeList [data-volume="${activeVolume}"]`);if(activeButton)activeButton.after(chapterBox);else box.prepend(chapterBox);chapterBox.style.cssText='margin:1px 0 1px 17px;padding:0;font-size:12px;line-height:16px';document.querySelector('#chapterPicker').style.cssText='padding:5px 8px;font-size:12px;line-height:16px';await loadVolume(tree,activeVolume);}
  const chapterDisplayName = (raw,index) => { const title=raw.replace(/^第(?:\d+|[一二三四五六七八九十百千零两]+)章[：:]?\s*/, '').trim() || '未命名'; return `第${index+1}章：${title}`; };
  async function loadVolume(tree,volume){if(!volume)return;const item=project(), proseRoot=tree.tree.find(node=>node.name==='正文'), node=prose(proseRoot).find(entry=>entry.name===volume), chapters=prose(node).filter(entry=>entry.type==='file'&&/\.txt$/i.test(entry.name)), previous=new Map(item.chapters.map(chapter=>[chapter.name,chapter]));item.files['正文'].splice(0,item.files['正文'].length);item.chapters=chapters.map((entry,index)=>({id:crypto.randomUUID(),name:entry.name.replace(/\.txt$/i,''),displayName:chapterDisplayName(entry.name.replace(/\.txt$/i,''),index),index:0,checked:false,approved:false,...(previous.get(entry.name.replace(/\.txt$/i,''))||{})}));const selected=chapters.find(entry=>entry.name===activeChapterFile)||chapters[0];activeChapterFile=selected?.name||'';item.chapterId=item.chapters.find(chapter=>`${chapter.name}.txt`===activeChapterFile)?.id||null;item.paths={...Object.fromEntries(Object.entries(item.paths||{}).filter(([,path])=>!path.startsWith('正文/')))};for(const entry of chapters){const path=`正文/${volume}/${entry.name}`,data=await request(`/api/file?project=${encodeURIComponent(item.diskName)}&path=${encodeURIComponent(path)}`);const key=window.novelLocal.add(item,'正文',path);item.content[key]=data.content;}document.querySelector('#chapterMenu').innerHTML=`${item.chapters.map(chapter=>`<button type="button" data-chapter="${chapter.displayName}" data-volume-chapter-file="${chapter.name}.txt">${chapter.displayName}</button>`).join('')}<button type="button" class="create-chapter" id="newChapterInVolume">＋ 新建章</button>`;document.querySelector('#chapterPicker').childNodes[0].nodeValue=`${item.chapters.find(chapter=>`${chapter.name}.txt`===activeChapterFile)?.displayName||'新建第1章'} `;renderFiles('正文');if(activeChapterFile)window.novelLocal?.selectChapterByFile?.(activeChapterFile,false);}
...
(() => {
  ...
  const chooseSource = () => { let input=document.querySelector('#localSourceUpload'); if(!input){ input=document.createElement('input'); input.id='localSourceUpload'; input.type='file'; input.accept='.txt,.md,text/plain,text/markdown'; input.className='hidden'; document.body.append(input); input.addEventListener('change',async()=>{ const item=project(), file=input.files[0]; if(!item||!file)return; if(!/\.(txt|md)$/i.test(file.name)){showToast('原著仅支持 .txt 或 .md 文件');input.value='';return;} const reader=new FileReader(); reader.onload=async()=>{ try { await request('/api/upload',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project:item.diskName,name:file.name,data:reader.result})}); item.sourceName=file.name; await window.novelLocal?.saveWorkflowState?.(item); showToast(`已切换原著：${file.name}`); } catch(error){ failure(error); } finally { input.value=''; } }; reader.readAsDataURL(file); }); } input.click(); };
