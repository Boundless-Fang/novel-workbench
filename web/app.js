...
(() => {
  ...
  function card(text, file, group, validation=false, draft=false) { const actions=validation?'<button type="button" data-new-action="undo">撤回</button><button type="button" data-new-action="retry">重试</button><button type="button" class="confirmed" data-new-action="pass">通过</button><button type="button" data-new-action="previous">上一步</button>':`<button type="button" data-new-action="undo">撤回</button><button type="button" data-new-action="retry">重试</button><button type="button" data-new-action="next">下一步</button>${draft?'<button type="button" data-new-action="draft">草稿</button>':''}`; clearPastStepActions(); messages.insertAdjacentHTML('beforeend',`<article class="message assistant-message completion"><div class="avatar">AI</div><div><p>${text}</p>${file?`<button class="generated-file" type="button" data-new-file="${file}" data-new-group="${group}"><span class="done-icon">✓</span> 已生成 <strong>${file}</strong><span class="open-arrow">打开 ›</span></button>`:''}<div class="message-actions">${actions}</div></div></article>`); messages.scrollTop=messages.scrollHeight; }
